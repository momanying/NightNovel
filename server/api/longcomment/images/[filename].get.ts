...
export default defineEventHandler(async (event) => {
  ...
  const imagePath = join(process.cwd(), 'storage', 'longcomment', filename)
  ...
  try {
    ...
    console.error(`Error serving long comment image ${filename}:`, error)
    throw createError({
      statusCode: 404,
      statusMessage: 'Long comment image not found'
    })
  }
}) 