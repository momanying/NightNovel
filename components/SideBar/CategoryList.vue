<template>
  <div class="bg-gray-800 rounded-lg overflow-hidden shadow-md">
    <h3 class="bg-gray-700 text-white font-medium text-lg py-3 px-4">小说分类</h3>
    <ul v-if="!loading" class="space-y-3 px-4 py-3">
      <li v-for="c in categories" :key="c.id">
        ...
        >
          {{ c.name }} 
          <span class="text-sm text-purple-300">({{ c.count }})</span>
        </a>
      </li>
    </ul>
...
defineProps<{
  categories: { id: number | string, name: string, count: number }[],
  loading: boolean,
  error: string
}>();
