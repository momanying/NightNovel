...
const novels = ref<Novel[]>([]); // Local state for novels
...
const fetchSimilarNovels = async () => {
    ...
    const sessionStorageKey = `similarNovels_${novelId.value}`;
    ...
    try {
        ...
        const response = await $fetch<{ data: Novel[] }>('/api/novels/similar', {
            query: params,
        });
