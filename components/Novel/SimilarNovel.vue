<template>
    <div v-if="novels.length > 0" class="w-full">
        <h2 class="title-h2">同类推荐</h2>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            <div v-for="novel in novels" :key="novel._id" class="flex flex-col cursor-pointer transition-transform duration-300 hover:-translate-y-1">
                <NuxtLink :to="`/novels/${novel._id}`">
                    <NuxtImg :src="novel.cover_url || '/placeholder.jpg'" :alt="novel.title" class="aspect-[3/4] w-full object-cover rounded-md" densities="x1 x2" />
...
// Get the similar novels state with the current novel ID
const { novels, set: setNovels, clear: clearNovels } = useSimilarNovels(novelId.value)
