<template>
    <div class="popular-ranking">
      ...
      <div class="flex justify-center items-end overflow-hidden border-b border-dashed border-gray-600 mb-4">
          <!-- 第二名 -->
          <div class="flex flex-col items-center relative w-1/3 cursor-pointer" @click="openNovelDetail(topNovels[1].id)">
          ...
          <!-- 第一名 -->
          <div class="flex flex-col items-center relative w-1/3 cursor-pointer" @click="openNovelDetail(topNovels[0].id)">
              <div class="w-[90px] h-[115px] relative mb-2.5 rounded-lg overflow-hidden shadow-md transition-transform duration-300 hover:-translate-y-1">
                  <img
                    class="w-full h-full object-cover" 
                    :src="topNovels[0].cover" 
          ...
          <!-- 第三名 -->
          <div class="flex flex-col items-center relative w-1/3 cursor-pointer" @click="openNovelDetail(topNovels[2].id)">
              <div class="w-[90px] h-[115px] relative mb-2.5 rounded-lg overflow-hidden shadow-md transition-transform duration-300 hover:-translate-y-1">
                  <img
                    class="w-full h-full object-cover" 
                    :src="topNovels[2].cover" 
      ...
      if(result.length < 10) {
          for(let i = result.length; i < 10; i++) {
              result.push({
                  ...
                  title: '暂无数据',
                  author: '-',
                  cover: 'https://placehold.co/200x300/cccccc/333333?text=暂无数据',
              });
          }
      }
